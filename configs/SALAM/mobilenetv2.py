...
def buildbody(options, system, clstr):
	...
	clstr.pwconv1 = CommInterface(devicename=acc, gic=gic, pio_addr=0x1004cd1b, pio_size=57)
...
def buildtail(options, system, clstr):
	...
	local_low = 0x100778c5
	local_high = 0x100cc537
	local_range = AddrRange(local_low, local_high)
	external_range = [AddrRange(0x00000000, local_low-1), AddrRange(local_high+1, 0xFFFFFFFF)]
	system.iobus.master = clstr.local_bus.slave
	clstr._connect_caches(system, options, l2coherent=False)
	...
	# Noncoherent DMA
	clstr.dma = NoncoherentDma(pio_addr=0x100778c5, pio_size = 21, gic=gic, int_num=95)
	clstr.dma.cluster_dma = clstr.local_bus.slave
	clstr.dma.max_req_size = 64
	clstr.dma.buffer_size = 128
	clstr.dma.dma = clstr.coherency_bus.slave
	...
	# Stream DMA
	clstr.stream_dma0 = StreamDma(pio_addr=0x100778da, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma0.stream_addr = 0x100778da + 32
	clstr.stream_dma0.stream_size = 8
	clstr.stream_dma0.pio_delay = '1ns'
	clstr.stream_dma0.rd_int = 210
	clstr.stream_dma0.wr_int = 211
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x10077902, pio_size=65, int_num=68)
	AccConfig(clstr.top, config, ir)
	...
	clstr.pwconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x10077943, pio_size=1)
	AccConfig(clstr.pwconv, config, ir)
	...
	clstr.reshape = CommInterface(devicename=acc, gic=gic, pio_addr=0x100c4835, pio_size=1)
	...
	clstr.avgpool = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc536, pio_size=1)
...
def buildclassifier(options, system, clstr):
	...
	local_low = 0x100cc538
	local_high = 0x10206e0f
	local_range = AddrRange(local_low, local_high)
	external_range = [AddrRange(0x00000000, local_low-1), AddrRange(local_high+1, 0xFFFFFFFF)]
	system.iobus.master = clstr.local_bus.slave
	clstr._connect_caches(system, options, l2coherent=False)
	...
	# Noncoherent DMA
	clstr.dma = NoncoherentDma(pio_addr=0x100cc538, pio_size = 21, gic=gic, int_num=95)
	clstr.dma.cluster_dma = clstr.local_bus.slave
	clstr.dma.max_req_size = 64
	clstr.dma.buffer_size = 128
	clstr.dma.dma = clstr.coherency_bus.slave
	...
	# Stream DMA
	clstr.stream_dma0 = StreamDma(pio_addr=0x100cc54d, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma0.stream_addr = 0x100cc54d + 32
	clstr.stream_dma0.stream_size = 8
	clstr.stream_dma0.pio_delay = '1ns'
	clstr.stream_dma0.rd_int = 210
	clstr.stream_dma0.wr_int = 211
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc575, pio_size=65, int_num=68)
	AccConfig(clstr.top, config, ir)
	...
	clstr.linear = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc5b6, pio_size=1)
