...
def buildhead(options, system, clstr):
	...
	clstr.stream_dma0.stream_size = 64
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x10020035, pio_size=65, int_num=68)
	...
	clstr.normalconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x10020076, pio_size=1)
	...
	clstr.dwconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x100205f2, pio_size=1)
	...
	clstr.pwconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x10020feb, pio_size=1)
	...
	addr = 0x100205ea
	clstr.normalconvout = StreamBuffer(stream_address = addr, stream_size = 8, buffer_size = 64)
	...
	addr = 0x10020fe3
	clstr.dwconvout = StreamBuffer(stream_address = addr, stream_size = 8, buffer_size = 64)
...
def buildbody(options, system, clstr):
	...
	local_low = 0x100211e5
	local_high = 0x100778cf
	local_range = AddrRange(local_low, local_high)
	external_range = [AddrRange(0x00000000, local_low-1), AddrRange(local_high+1, 0xFFFFFFFF)]
	system.iobus.master = clstr.local_bus.slave
	clstr._connect_caches(system, options, l2coherent=False)
	...
	# Noncoherent DMA
	clstr.dma = NoncoherentDma(pio_addr=0x100211e5, pio_size = 21, gic=gic, int_num=96)
	clstr.dma.cluster_dma = clstr.local_bus.slave
	clstr.dma.max_req_size = 64
	clstr.dma.buffer_size = 128
	clstr.dma.dma = clstr.coherency_bus.slave
	...
	# Stream DMA
	clstr.stream_dma0 = StreamDma(pio_addr=0x100211fa, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma0.stream_addr = 0x100211fa + 32
	clstr.stream_dma0.stream_size = 64
	clstr.stream_dma0.pio_delay = '1ns'
	...
	# Stream DMA
	clstr.stream_dma1 = StreamDma(pio_addr=0x1002121a, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma1.stream_addr = 0x1002121a + 32
	clstr.stream_dma1.stream_size = 64
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x1002123a, pio_size=81, int_num=68)
	AccConfig(clstr.top, config, ir)
	...
	clstr.residual = CommInterface(devicename=acc, gic=gic, pio_addr=0x1002128b, pio_size=17)
	...
	clstr.pwconv0 = CommInterface(devicename=acc, gic=gic, pio_addr=0x100212a4, pio_size=57)
	...
	clstr.dwconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x100375bd, pio_size=65)
	AccConfig(clstr.dwconv, config, ir)
	...
	clstr.pwconv1 = CommInterface(devicename=acc, gic=gic, pio_addr=0x1004cd26, pio_size=57)
	...
	addr = 0x1002129c
	clstr.residualout = StreamBuffer(stream_address = addr, stream_size = 8, buffer_size = 64)
	...
	addr = 0x100375b5
	clstr.pwconv0out = StreamBuffer(stream_address = addr, stream_size = 8, buffer_size = 64)
	...
	# DWConvWindow (Variable)
	addr = 0x100375fe
	spmRange = AddrRange(addr, addr + 0x1950)
	clstr.dwconvwindow = ScratchpadMemory(range = spmRange)
	clstr.dwconvwindow.conf_table_reported = False
	clstr.dwconvwindow.ready_mode = False
	clstr.dwconvwindow.port = clstr.local_bus.master
	...
	addr = 0x1003bc4e
	spmRange = AddrRange(addr, addr + 0x10e00)
...
def buildtail(options, system, clstr):
	...
	local_low = 0x100778d0
	local_high = 0x100cc541
	local_range = AddrRange(local_low, local_high)
	external_range = [AddrRange(0x00000000, local_low-1), AddrRange(local_high+1, 0xFFFFFFFF)]
	system.iobus.master = clstr.local_bus.slave
	clstr._connect_caches(system, options, l2coherent=False)
	...
	# Noncoherent DMA
	clstr.dma = NoncoherentDma(pio_addr=0x100778d0, pio_size = 21, gic=gic, int_num=95)
	clstr.dma.cluster_dma = clstr.local_bus.slave
	clstr.dma.max_req_size = 64
	clstr.dma.buffer_size = 128
	clstr.dma.dma = clstr.coherency_bus.slave
	...
	# Stream DMA
	clstr.stream_dma0 = StreamDma(pio_addr=0x100778e5, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma0.stream_addr = 0x100778e5 + 32
	clstr.stream_dma0.stream_size = 64
	clstr.stream_dma0.pio_delay = '1ns'
	clstr.stream_dma0.rd_int = 210
	clstr.stream_dma0.wr_int = 211
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x10077905, pio_size=65, int_num=68)
	AccConfig(clstr.top, config, ir)
	...
	clstr.pwconv = CommInterface(devicename=acc, gic=gic, pio_addr=0x10077946, pio_size=1)
	AccConfig(clstr.pwconv, config, ir)
	...
	clstr.reshape = CommInterface(devicename=acc, gic=gic, pio_addr=0x100c483f, pio_size=1)
	...
	clstr.avgpool = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc540, pio_size=1)
	...
	addr = 0x100c4837
	clstr.pwconvout = StreamBuffer(stream_address = addr, stream_size = 8, buffer_size = 64)
...
def buildclassifier(options, system, clstr):
	...
	local_low = 0x100cc542
	local_high = 0x10206e11
	local_range = AddrRange(local_low, local_high)
	external_range = [AddrRange(0x00000000, local_low-1), AddrRange(local_high+1, 0xFFFFFFFF)]
	system.iobus.master = clstr.local_bus.slave
	clstr._connect_caches(system, options, l2coherent=False)
	...
	# Noncoherent DMA
	clstr.dma = NoncoherentDma(pio_addr=0x100cc542, pio_size = 21, gic=gic, int_num=95)
	clstr.dma.cluster_dma = clstr.local_bus.slave
	clstr.dma.max_req_size = 64
	clstr.dma.buffer_size = 128
	clstr.dma.dma = clstr.coherency_bus.slave
	...
	# Stream DMA
	clstr.stream_dma0 = StreamDma(pio_addr=0x100cc557, pio_size = 32, gic=gic, max_pending = 32)
	clstr.stream_dma0.stream_addr = 0x100cc557 + 32
	clstr.stream_dma0.stream_size = 64
	clstr.stream_dma0.pio_delay = '1ns'
	clstr.stream_dma0.rd_int = 210
	clstr.stream_dma0.wr_int = 211
	...
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc577, pio_size=65, int_num=68)
	AccConfig(clstr.top, config, ir)
	...
	clstr.linear = CommInterface(devicename=acc, gic=gic, pio_addr=0x100cc5b8, pio_size=1)
