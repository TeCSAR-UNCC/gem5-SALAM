...
void DWConv() {
    ...
    dType_Reg	read_count 	= (DW0_0_I_SIZE * HALF_SIZE + HALF_SIZE + 1) * DW0_0_IC_SIZE;

// Warmup
buf_x1_n:
	#pragma clang loop unroll(disable)
    for (dType_Reg x = DW0_0_I_SIZE - HALF_SIZE - 1; x < DW0_0_I_SIZE; x++) {
    itr_channel_x1:
		#pragma clang loop unroll(disable)
        for (dType_Reg ch = 0; ch < DW0_0_IC_SIZE; ch++) {
            // BUFFER[ch][HALF_SIZE - 1][x] = 1;
            BUFFER[ch*(t_KSIZE-1)*DW0_0_I_SIZE + (HALF_SIZE-1)*DW0_0_I_SIZE + x] = *STR_IN;
        }
    }
...
buf_y_n:
    ...
    buf_x2_n:
		#pragma clang loop unroll(disable)
        for (dType_Reg x = 0; x < DW0_0_I_SIZE; x++) {
        itr_channel_x2:
			#pragma clang loop unroll(disable)
            for (dType_Reg ch = 0; ch < DW0_0_IC_SIZE; ch++) {
                // BUFFER[ch][y][x] = 1;
                BUFFER[ch*(t_KSIZE-1)*DW0_0_I_SIZE + y*DW0_0_I_SIZE + x] = *STR_IN;
            }
        }
    }
    ...
    win_x_n:
        ...
        win_itr_channel:
            ...
            for (dType_Reg ch = 0; ch < DW0_0_IC_SIZE; ch++) {
                ...
                WINDOW[ch*t_KSIZE*t_KSIZE + y*t_KSIZE + x] = BUFFER[ch*(t_KSIZE-1)*DW0_0_I_SIZE + (y-1)*DW0_0_I_SIZE + (x+DW0_0_I_SIZE-t_KSIZE)];
            }
        }
    }
...
// Start Convolution
    dType_Reg yVal, xVal;
    if (DW0_0_STRIDE == 2) {
        x_looper = y_looper = DW0_0_O_SIZE * 2;
    } else {
        x_looper = y_looper = DW0_0_O_SIZE;
    }
...
for_y_n:
    ...
    for_x_n:
    	...
    	compute_loop_channel_n:
			#pragma clang loop unroll(disable)
        	for (dType_Reg oc = 0; oc < DW0_0_OC_SIZE; oc++) {
        		bias = BIAS[oc];
			...
			win_i_n:
			    ...
			    win_j_n:
			        ...
			        for (int j = -HALF_SIZE; j <= HALF_SIZE; j++) {
		                if (0 <= (y+i) && (y+i) < DW0_0_I_SIZE && 0 <= (x+j) && (x+j) < DW0_0_I_SIZE) {
		                    // dType_8t actInp = window[oc][i + HALF_SIZE][j + HALF_SIZE] - input_zp;
		                    dType_8t actInp = WINDOW[oc*t_KSIZE*t_KSIZE + (i + HALF_SIZE)*t_KSIZE + (j + HALF_SIZE)] - DW0_0_IN_ZP;
        		...
        		dType_16t bias_calib = bias - DW0_0_BIAS_ZP;
        		...
        		out_i32 = (out_i32 >> NSHIFTOUT[oc]) + DW0_0_OUT_ZP;
		...
		Data_movement_loop:
		    ...
		    shift_win_y:
		        ...
		        WINDOW[ch*t_KSIZE*t_KSIZE + 0*t_KSIZE + (t_KSIZE-1)] = BUFFER[ch*(t_KSIZE-1)*DW0_0_I_SIZE + 0*DW0_0_I_SIZE + x];
		    update_idx1:
				#pragma clang loop unroll(full)
		        for (int y = 1; y < t_KSIZE - 1; y++) {
		            ...
		            int buffidx1 = ch*(t_KSIZE-1)*DW0_0_I_SIZE + (y-1)*DW0_0_I_SIZE + x;
		            int buffidx2 = ch*(t_KSIZE-1)*DW0_0_I_SIZE + y*DW0_0_I_SIZE + x;
		            WINDOW[winidx] = BUFFER[buffidx1] = BUFFER[buffidx2];
		        }

		        dType_8u val_in = 0;
		        if (read_count < DW0_0_I_SIZE * DW0_0_I_SIZE * DW0_0_IC_SIZE) {
		            val_in = *STR_IN;
		            read_count++;
		        }
		        // WINDOW[ch][t_KSIZE - 1][t_KSIZE - 1] = BUFFER[ch][t_KSIZE - 2][x] = val_in;
		        int winidx = ch*t_KSIZE*t_KSIZE + (t_KSIZE-1)*t_KSIZE + (t_KSIZE-1);
		        int buffidx = ch*(t_KSIZE-1)*DW0_0_I_SIZE + (t_KSIZE-2)*DW0_0_I_SIZE + x;
