...
#define loc_sol      *(int *)(0x2f000001 + sizeof(TYPE))
#define loc_filter   *(int *)(0x2f000001 + 2*sizeof(TYPE))
...
void genData(stencil_struct * sts) {
    ...
    for(k1=0; k1 < FIL; k1++) {
        for (k2=0; k2 < FIL; k2++){    
            sts->filter[k1*FIL + k2] = k1*FIL + k2;
        }
    }
    ...
    for( r=0; r < ROW; r++ ) {
        for( c=0; c < COL; c++ ) {
            ...
            for(k1=0; k1 < FIL; k1++) {
                for (k2=0; k2 < FIL; k2++){                    
                    mul = sts->filter[k1*FIL + k2] * sts->inp[(r+k1)*COL + c + k2];
                    temp += mul;
                }
            }
